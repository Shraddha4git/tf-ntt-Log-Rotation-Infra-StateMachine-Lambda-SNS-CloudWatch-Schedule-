...
def lambda_handler(event, context):
    try:
        # Run the restart script for TG1
        command_id = run_ssm_command(TG1_INSTANCE_IDS, COMMAND)
        event["CommandId"] = command_id
        event["s4-ScriptExecutionStatus"] = "Success"
    except Exception as e:
        event["s4-ScriptExecutionStatus"] = "Fail"
    return event
